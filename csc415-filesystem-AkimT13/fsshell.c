...
int cmd_mv(int argcnt, char *argvec[])
	{
		...
		char *src = argvec[1];  // Removed const to match function signatures
		char *dest = argvec[2]; // Removed const to match function signatures

		// Check if source exists and is a regular file
		if (!fs_isFile(src)) {
			printf("mv: cannot move '%s': No such file\n", src);
			return -1;
		}

		// Check if destination exists
		if (fs_isFile(dest)) {
			printf("mv: cannot move to '%s': File exists\n", dest);
			return -1;
		}

		// Since fs_rename doesn't exist, we'll use copy+delete method
		int src_fd = b_open(src, O_RDONLY);
		if (src_fd < 0) {
			printf("mv: cannot open '%s': Error %d\n", src, src_fd);
			return -1;
		}

		int dest_fd = b_open(dest, O_WRONLY | O_CREAT | O_EXCL);
		if (dest_fd < 0) {
			printf("mv: cannot create '%s': Error %d\n", dest, dest_fd);
			b_close(src_fd);
			return -1;
		}
		...
		ssize_t bytes_read;
		while ((bytes_read = b_read(src_fd, buffer, sizeof(buffer)))) {
			if (bytes_read < 0) {
				printf("mv: read error\n");
				goto error_cleanup;
			}

			if (b_write(dest_fd, buffer, bytes_read) != bytes_read) {
		...
		if (fs_delete(src) != 0) {
			printf("mv: warning: failed to remove original file\n");
			// Still return success since copy worked
		}
		...
		error_cleanup:
			b_close(src_fd);
			b_close(dest_fd);
			fs_delete(dest);  // Remove partial file
			return -1;
	}
